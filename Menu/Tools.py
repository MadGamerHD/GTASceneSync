        ...
        for obj in selected_objects:
            if obj.type == 'MESH':  # Assuming we only want to change mesh objects
                ...
                if not hasattr(obj, 'dff'):
                    obj.dff = bpy.props.PointerProperty(type=bpy.types.PropertyGroup)  # Create a custom property group if needed
...
class VIEW3D_PT_batch_rename_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(context.scene, "batch_rename_base_name", text="Base Name")  # Property input for base name
